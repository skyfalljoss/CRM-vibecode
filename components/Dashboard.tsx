...
const Dashboard: React.FC<DashboardProps> = ({ leads, stages, activities }) => {
  const kpis = useMemo(() => {
    ...
    const activeLeads = leads.filter(l => l.stage_id !== wonStage?.id && l.stage_id !== lostStage?.id).length;
  ...
  const pipelineData = useMemo(() => {
    return stages.map(stage => ({
      ...
      amount: leads.filter(l => l.stage_id === stage.id).reduce((sum, l) => sum + Number(l.value), 0),
      color: stage.color
    }));
  }, [leads, stages]);
  ...
  const todayActions = useMemo(() => {
    ...
    return activities.filter(a => new Date(a.created_at).toDateString() === today).length;
  }, [activities]);

  const progressPercentage = Math.min((todayActions / todayGoal) * 100, 100);
  ...
  const activityData = useMemo(() => {
    ...
    activities.forEach(act => {
      ...
      const dayStat = last7Days.find(d => d.date === actDate);
      if (dayStat) {
        dayStat.actions++;
      }
    });
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="glass-card px-4 py-2 rounded-lg flex items-center gap-3">
          <div className={`w-2 h-2 rounded-full ${todayActions >= todayGoal ? 'bg-green-500' : 'bg-amber-500'} animate-pulse`} />
          <span className="text-xs font-medium text-zinc-300">Daily Goal: {todayActions}/{todayGoal} Actions</span>
          <div className="w-24 h-1 bg-white/5 rounded-full overflow-hidden">
            <div
              className={`h-full transition-all duration-500 ${todayActions >= todayGoal ? 'bg-green-500' : 'bg-amber-500'}`}
            ...
            activities.slice(0, 5).map((act, i) => (
              <div key={i} className="flex items-start gap-4 text-xs border-b border-white/5 pb-4 last:border-0 last:pb-0">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-zinc-500 text-[10px] mt-0.5">{act.created_at ? new Date(act.created_at).toLocaleTimeString() : 'Recently'}</p>
                </div>
              </div>
            ))
          )}
